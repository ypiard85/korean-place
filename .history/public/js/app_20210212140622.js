...
function Slide(){
 ...
 function  WeatherApi(){
    let citys = ['Seoul', 'Busan', 'Incheon', 'Jeju', 'ulsan']
        citys.forEach(el => {
           window.onload.fetch(`http://api.openweathermap.org/data/2.5/weather?q=${el}&units=metric&appid=2b1ec7a5b325cfbf7c1bc18b936df3f3`).then( res => res.json()).then(CityItems)
        })
